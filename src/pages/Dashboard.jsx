  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Vue d'ensemble — {MONTHS[month - 1]} {year} · Solde total : {formatMGA(soldeTempsReel)}
          </p>
        </div>
      ...
      {alertesBudget.length > 0 && (
        <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-2xl p-4 flex items-start gap-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <KpiCard label="Reste à vivre" value={formatMGA(kpis.reste)} icon={Coins} tone="primary" />
      </div>
      ...
      {forecast.finDeMoisDepenses !== null && (
        <div className="bg-card rounded-2xl border border-border p-4 shadow-sm flex items-center gap-3">
          <span className="h-9 w-9 rounded-xl bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 flex items-center justify-center shrink-0">
            <TrendingUp className="h-4.5 w-4.5" />
          </span>
          <p className="text-sm">
            Au rythme actuel, vos dépenses de {MONTHS[month - 1]} devraient atteindre environ{" "}
            <span className="font-semibold">{formatMGA(forecast.finDeMoisDepenses)}</span> en fin de mois.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
          <h2 className="font-heading font-semibold">Répartition des dépenses</h2>
          <p className="text-xs text-muted-foreground mb-3">Détail par catégorie</p>
          <ExpensePieChart data={depensesParCategorie} />
        </div>
        <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
          <h2 className="font-heading font-semibold">Budget prévu vs réel</h2>
